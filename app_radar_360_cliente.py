...
CONTROLADOR_COOKIES = CookieController()
NOME_COOKIE_ACESSO = "radar360_cliente_autenticado"
...
def _token_acesso_cliente(senha_configurada):
    chave_cookie = str(
        st.secrets.get("acesso_cliente", {}).get(
            "chave_cookie",
            senha_configurada,
        )
    ).strip()
    ...
    return hashlib.sha256(
        f"{senha_configurada}|{chave_cookie}".encode("utf-8")
    ).hexdigest()
...
def verificar_acesso_cliente():
    ...
    token_esperado = _token_acesso_cliente(senha_configurada)

    cookies = CONTROLADOR_COOKIES.getAll()
    if cookies is None:
        tentativas = st.session_state.get("tentativas_carregar_cookies", 0)
        if tentativas < 2:
            st.session_state["tentativas_carregar_cookies"] = tentativas + 1
            time.sleep(0.25)
            st.rerun()
        cookies = {}

    token_cookie = cookies.get(NOME_COOKIE_ACESSO) if isinstance(cookies, dict) else None

    if token_cookie == token_esperado:
        st.session_state["cliente_autenticado"] = True
        st.session_state.pop("tentativas_carregar_cookies", None)
    ...
    st.caption(
        "Após entrar, o acesso permanecerá ativo neste navegador por até 8 horas."
    )
...
def renderizar_botao_sair():
    ...
    if st.sidebar.button(
        ...
        key="botao_sair_cliente",
    ):
        st.session_state["cliente_autenticado"] = False
        st.session_state.pop("senha_acesso_cliente", None)

        CONTROLADOR_COOKIES.remove(
            NOME_COOKIE_ACESSO
        )
